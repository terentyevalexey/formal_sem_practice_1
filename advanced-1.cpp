...
void func(void* func_ptr, int argc, void* out_buffer, int& offset) {
    ...
    unsigned* ptr = int_buffer + offset + 5;                             // 5 because we wrote 5 instructions to buffer
    unsigned nxt_instruction = *reinterpret_cast<unsigned*>(&ptr);       // nxt_instruction pointer
    unsigned right_16 = nxt_instruction & 0xFFFFu;
    unsigned mask = (right_16 & 0xFFFu) | ((right_16 << 4u) & 0xF0000u);
    int_buffer[offset++] = 0xe3a0e000u | mask;                           // movw lr, nxt_instruction::lower_16
    unsigned left_16 = nxt_instruction >> 16u;
    mask = (left_16 & 0xFFFu) | ((left_16 << 4u) & 0xF0000u);
    int_buffer[offset++] = 0xe340e000u | mask;                           // movt lr, nxt_instruction::upper_16
    ...
    int_buffer[offset++] = 0xe3a04000u | mask;                           // movw r4, func_pointer::lower_16
    left_16 = func_pointer >> 16u;
    mask = (left_16 & 0xFFFu) | ((left_16 << 4u) & 0xF0000u);
    int_buffer[offset++] = 0xe3404000u | mask;                           // movt r4, func_pointer::upper_16
...
void constant(const std::string& str, void* out_buffer, int& offset) {
    ...
    unsigned mask = (right_16 & 0xFFFu) | ((right_16 << 4u) & 0xF0000u);
    int_buffer[offset++] = 0xe3a00000u | mask;                           // movw r0, int_constant::lower_16
    unsigned left_16 = unsigned_constant >> 16u;
    mask = (left_16 & 0xFFFu) | ((left_16 << 4u) & 0xF0000u);
    int_buffer[offset++] = 0xe3400000u | mask;                           // movt r0, int_constant::upper_16
    int_buffer[offset++] = 0xe52d0004u;  // push {r0}
...
void external_int(void* ptr, void* out_buffer, int& offset) {
    ...
    unsigned mask = (right_16 & 0xFFFu) | ((right_16 << 4u) & 0xF0000u);
    int_buffer[offset++] = 0xe3a00000u | mask;                           // movw r0, external_ptr::lower_16
    unsigned left_16 = external_ptr >> 16u;
    mask = (left_16 & 0xFFFu) | ((left_16 << 4u) & 0xF0000u);
    int_buffer[offset++] = 0xe3400000u | mask;                           // movt r0, external_ptr::upper_16
...
void pop_all_operators(std::stack<std::pair<char, void*>>& sym_stack, void* out_buffer, int offset) {
...
void pop_all_multiplies(std::stack<std::pair<char, void*>>& sym_stack, void* out_buffer, int offset) {
...
void jit_compile_expression_to_arm(const char* expression,
    ...
    while ((cur_char = *expression) != '\0') {
        if (cur_char == ' ')                                    // ignore spaces
            continue;
        if (!is_close_bracket(cur_char) && !args_cnt.empty() && args_cnt.top() == 0)      // COUNT FIRST ARG
            ++args_cnt.top();
        if (is_digit(cur_char)) {                               // EXTERNAL CONTINUATION OR CONSTANT PART
            ...
            else if (cur_token_t == EXTERNAL) {                 // before operator was external (int)
            ...
            cur_token.clear();
            if (is_plus(cur_char) || is_minus(cur_char))        // minus or plus (least priority operators)
                pop_all_operators(sym_stack, out_buffer, offset);
            else if (is_multiply(cur_char))                     // multiply (most priority operator)
            ...
            if (cur_token_t == EXTERNAL) {                      // before open bracket was external (func)
                symbol_t cur_symbol = *externs;
                while (cur_symbol.name != cur_token)            // finding name by token
                    cur_symbol = *(++externs);
            ...
            if (cur_token_t == CONSTANT)                        // before close bracket was constant
                constant(cur_token, out_buffer, offset);
            else if (cur_token_t == EXTERNAL) {                 // before close bracket was external (int)
                symbol_t cur_symbol = *externs;
                while (cur_symbol.name != cur_token)            // finding name by token
                    cur_symbol = *(++externs);
                external_int(cur_symbol.pointer, out_buffer, offset);
            ...
            pop_all_operators(sym_stack, out_buffer, offset);
            sym_stack.pop();
            if (sym_stack.top().second) {                       // func before brackets
                func(sym_stack.top().second, args_cnt.top(), out_buffer, offset);
                args_cnt.pop();
                sym_stack.pop();
            }
            cur_token_t = NOTHING;
            ...
            if (cur_token_t == CONSTANT)                        // before comma was constant
                constant(cur_token, out_buffer, offset);
            else if (cur_token_t == EXTERNAL) {                 // before comma was external (int)
                symbol_t cur_symbol = *externs;
                while (cur_symbol.name != cur_token)            // finding name by token
                    cur_symbol = *(++externs);
                external_int(cur_symbol.pointer, out_buffer, offset);
